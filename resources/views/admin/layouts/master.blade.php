...
<body id="kt_body"
        ...
        <div class="page d-flex flex-row flex-column-fluid">
            ...
            <div class="wrapper d-flex flex-column flex-row-fluid" id="kt_wrapper">
                ...
                <div class="content d-flex flex-column flex-column-fluid" id="kt_content">
                    @include('admin.layouts.toolbar', ['slug' => get_slug()])
    ...
    <script src="{{ asset('admin/assets/js/filepond/filepond.media.preview.js') }}"></script>
    <script src="{{ asset('admin/assets/js/filepond/filepond.metadata.js') }}"></script>
    ...
    <script src="{{ asset('admin/assets/js/repeater.min.js') }}"></script>
    ...
    @yield('js')
    <script>
        $(document).delegate("#files-container .item", "click", function() {
            if ($(this).data('type') == 'directory') {
                let folder = $(this).data('folder')
                $.ajax({
                    type: "GET",
                    url: '/dashboard/get-modal-file/' + folder,
                    success: function(response) {
                        $('.main-wrapper').html(response)
                    },
                    error: function(response) {
                        /* data = response.responseJSON.data ?? response.responseJSON
                        alert_function(data);
                        generate_recaptcha() */
                    }
                });
